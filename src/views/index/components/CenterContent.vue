...
<!--        <button @click="robotMove">test</button>-->
        <!--<button @click="addV">加速</button>
        <button @click="deleteV">减速</button>
...
<script>
    let map ,viewer ,topLine ,bottomLine ,robot ;
    ...
    export default {
        ...
        data() {
            return {
                ...
                currentTower: [],
                vcLayer: null,
                robotUrl: 'http://192.168.1.242:3000/model/robot-processed.glb',
                towerUrl: 'http://192.168.1.242:3000/model/tower-processed.glb'
            }
        },
        watch: {
        ...
        destroyed(){
          map = null;
          const threeMap = document.querySelector("#threeMap");
            threeMap.removeChild(threeMap.childNodes[0]);
            threeMap.removeChild(threeMap.childNodes[1]);
        },
        methods: {
            //  2d 地图与视频切换
            twoMapShow() {
            ...
            webglInit() {
                ...
                // 对三维地图进行设置 电塔模型 机器人运动等
                this.setThreeMap();
                //this.robotMove();
            },
            setThreeMap(){
                const target = Cesium.Cartesian3.fromDegrees(testPointData[0].position[0], testPointData[0].position[1], 50.5);
            ...
            robotMove(){
                ...
                let stop = Cesium.JulianDate.addSeconds(start, (testPointData.length), new Cesium.JulianDate())
                ...
                let num = 0;
                property.addSample(Cesium.JulianDate.addSeconds(start, num, new Cesium.JulianDate()),
                    Cesium.Cartesian3.fromDegrees(testPointData[0].position[0], testPointData[0].position[1], 48));
                for (let i of testPointData) {
                    // 增加机器人运动轨迹以及机器人高度
                    property.addSample(Cesium.JulianDate.addSeconds(start, num += 1, new Cesium.JulianDate()),
                        Cesium.Cartesian3.fromDegrees(i.position[0], i.position[1], 48));
                }
                robot.position = property;
                robot.orientation = new Cesium.VelocityOrientationProperty(property);
                robot.availability = new Cesium.TimeIntervalCollection([new Cesium.TimeInterval({
                    start: start,
                    stop: stop
                })]);
